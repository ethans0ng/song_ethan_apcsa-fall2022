...
//  } Not needed I guess
  ...
  public int findClosestFib(int n) {
	  int[] numbers = new int[]{0, 1, 2, 3, 5, 8, 13, 21, 34, 55, 66, 78, 89, 144, 151, 158, 233, 255, 254, 253, 252
			  , 250, 247, 242, 234, 221, 214, 207, 200, 189, 177, 166, 122, 133, 111, 22};
	  //207, 214, 122, 133, 166, 177, 66, 78, 151, 158
	  int residual = 1000, counter = 0;
	  for(int i=0; i<numbers.length; i++) {
		  if(Math.abs(numbers[i]-n)<=residual) {
			  residual = Math.abs(numbers[i]-n);
			  counter = i;
		  }
	  }
  ...
  public void encodeFib(Picture messagePict)
  {
	  Pixel[][] messagePixels = messagePict.getPixels2D();
	  Pixel[][] currPixels = this.getPixels2D();
	  Pixel currPixel = null;
	  ...
	  int height = Math.min(this.getHeight(), messagePict.getHeight());
	  int width = Math.min(this.getWidth(), messagePict.getWidth());
	  ...
	  for (int row = 0; row < height; row++)
	  {
		  for (int col = 0; col < width; col++)
		  {
			  ...
			  if (messagePixel.colorDistance(Color.BLACK) < 50)
			  {
				  currPixel.setRed(findClosestFib(currPixel.getRed()));
				  currPixel.setBlue(findClosestFib(currPixel.getBlue()));
				  currPixel.setGreen(findClosestFib(currPixel.getGreen()));
			  }
			  else {
				  boolean up = true;
				  while(findClosestFib(currPixel.getRed())==currPixel.getRed()&&findClosestFib(currPixel.getBlue())==
						  currPixel.getBlue()&&findClosestFib(currPixel.getGreen())==currPixel.getGreen()) {
					  if(currPixel.getRed()==255 || currPixel.getBlue()==255 || currPixel.getGreen()==255) {
						  up=false;
					  }
					  if(up) {
						  currPixel.setRed(currPixel.getRed()+1);
  ...
  public Picture decodeFib()
  {
	  ...
	  for (int row = 0; row < this.getHeight(); row++)
	  {
		  for (int col = 0; col < this.getWidth(); col++)
		  {
			  currPixel = pixels[row][col];
			  messagePixel = messagePixels[row][col];
			  if (findClosestFib(currPixel.getRed())==currPixel.getRed()&&findClosestFib(currPixel.getBlue())==
					  currPixel.getBlue()&&findClosestFib(currPixel.getGreen())==currPixel.getGreen())
			  {
				  messagePixel.setColor(Color.BLACK);
				  count++;
